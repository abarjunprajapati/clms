...
if (!enrolment_column_exists($conn, 'workmen', 'expected_joining_date')) {
...
function renderContent() {
    ...
    if ($role === 'customer') {
        $customerCode = $_SESSION['customer_code'] ?? $_SESSION['contractor_id'] ?? '';
        $customerContractorIds = $customerContractorIds ?? enrolment_customer_contractor_ids($conn, $customerCode);
    ...
    if ($role === 'customer') {
        $vendorCodeForSap = $_SESSION['customer_code'] ?? $vendorCodeForSap;
    }
    $workOptions = [];
    $seenWorkOrders = [];
    ...
    if ($c_id && enrolment_table_exists($conn, 'workmen')) {
        ...
        $workers = enrolment_fetch_all($conn, "
            SELECT
                ...
                '' AS bank_account,
                '' AS ifsc,
                " . enrolment_expr($conn, 'workmen', 'expected_joining_date', 'expected_joining_date') . ",
                ...
                (
                    ...
                    WHERE tbw.workman_id = workmen.id
                      AND tbw.ticked = 1
                    ORDER BY tbw.id DESC
                    LIMIT 1
                ) AS latest_training_batch,
                (
                    SELECT cb.training_date
                    FROM training_batch_workers tbw
                    JOIN training_class_batches cb ON cb.id = tbw.batch_id
                    WHERE tbw.workman_id = workmen.id
                      AND tbw.ticked = 1
                ...
                (
                    SELECT EXISTS (
                        ...
                        WHERE pw.workman_id = workmen.id
                          AND pr.status = 'paid'
                    )
                ) AS is_safety_paid,
    ...
    // Fetch dynamic documents for all workers currently loaded
    $workerIds = array_column($workers, 'id');
    if ($autoEditWorker) {
        $workerIds[] = (int)$autoEditWorker['id'];
    }
    $workerIds = array_unique(array_filter($workerIds));
    $workerDocs = [];
    if (!empty($workerIds)) {
        $idsCsv = implode(',', array_map('intval', $workerIds));
        ...
        while ($docRow = mysqli_fetch_assoc($docRes)) {
            $workerDocs[$docRow['workman_id']][$docRow['document_type']] = basename($docRow['file_path']);
        }
    }
    ...
    unset($w);
    if ($autoEditWorker) {
        $autoEditWorker['documents'] = $workerDocs[$autoEditWorker['id']] ?? [];
    }
    ?>
    <style>
    .invalid-border {
    ...
    .preview-section-title { padding:10px 12px; background:#1e3a8a; border-bottom:1px solid #1e3a8a; font-size:11px; font-weight:900; color:#ffffff; text-transform:uppercase; }
    ...
    /* .preview-doc-view now uses global .btn-view from style.css */
    ...
    <div class="card shadow-sm mb-4" style="border-radius: 12px; border: 1px solid #e2e8f0; overflow: hidden; background: #ffffff;">
      ...
      <div class="card-body p-3">
        <div class="row g-3">
          ...
          <div class="col-lg-7">
            <div class="stats-grid" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; height: 100%;">
              <div class="stat-card glass" style="padding:10px; min-height:0; display:flex; align-items:center; gap:8px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc;">
                <div class="stat-icon" style="background:rgba(99,102,241,0.1);color:#6366f1; width:32px; height:32px; font-size:14px; flex-shrink:0; display:flex; align-items:center; justify-content:center; border-radius:50%;"><i class="fas fa-users"></i></div>
                <div>
                  <div class="stat-value" style="font-size:18px; font-weight:700; color:#1e293b; line-height:1; margin-bottom:2px;"><?= count($workers) ?></div>
                  <div class="stat-label" style="font-size:10px; color:#64748b; font-weight:500;"><?= htmlspecialchars($selectedType['plural']) ?> Enrolled</div>
                </div>
              </div>
              <div class="stat-card glass" style="padding:10px; min-height:0; display:flex; align-items:center; gap:8px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc;">
                <div class="stat-icon" style="background:rgba(16,185,129,0.1);color:#10b981; width:32px; height:32px; font-size:14px; flex-shrink:0; display:flex; align-items:center; justify-content:center; border-radius:50%;"><i class="fas fa-check"></i></div>
                <div>
                  <div class="stat-value" style="font-size:18px; font-weight:700; color:#1e293b; line-height:1; margin-bottom:2px;"><?= count(array_filter($workers, function($w) { return ($w['safety_status']??'')==='pass'; })) ?></div>
                  <div class="stat-label" style="font-size:10px; color:#64748b; font-weight:500;">Safety Passed</div>
                </div>
              </div>
              <div class="stat-card glass" style="padding:10px; min-height:0; display:flex; align-items:center; gap:8px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc;">
                <div class="stat-icon" style="background:rgba(245,158,11,0.1);color:#f59e0b; width:32px; height:32px; font-size:14px; flex-shrink:0; display:flex; align-items:center; justify-content:center; border-radius:50%;"><i class="fas fa-id-badge"></i></div>
                <div>
                  <div class="stat-value" style="font-size:18px; font-weight:700; color:#1e293b; line-height:1; margin-bottom:2px;"><?= count(array_filter($workers, function($w) { return !empty($w['temp_id']); })) ?></div>
                  <div class="stat-label" style="font-size:10px; color:#64748b; font-weight:500;">Temp IDs Issued</div>
                </div>
              </div>
              <div class="stat-card glass" style="padding:10px; min-height:0; display:flex; align-items:center; gap:8px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc;">
                <div class="stat-icon" style="background:rgba(239,68,68,0.1);color:#ef4444; width:32px; height:32px; font-size:14px; flex-shrink:0; display:flex; align-items:center; justify-content:center; border-radius:50%;"><i class="fas fa-door-open"></i></div>
                <div>
                  <div class="stat-value" style="font-size:18px; font-weight:700; color:#1e293b; line-height:1; margin-bottom:2px;"><?= count(array_filter($workers, function($w) { return ($w['gate_pass_status']??'')==='active'; })) ?></div>
                  <div class="stat-label" style="font-size:10px; color:#64748b; font-weight:500;">Active Passes</div>
                </div>
              </div>
            </div>
          </div>
          <!-- Pass Limits Column -->
          <div class="col-lg-5">
            <div class="p-2 border rounded-3 bg-light" id="passLimitsWidget" style="font-size:12px; height: 100%; min-height: 56px;">
              <p class="text-muted mb-0">Loading pass limits...</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style="min-width: 220px;">
                <?php if (!empty($w['is_blocked'])): ?>
                  <div style="margin-bottom: 8px; padding: 6px; background: #fee2e2; border: 1px solid #fca5a5; border-radius: 6px;">
                    <div style="color: #b91c1c; font-weight: bold; font-size: 12px;"><i class="fas fa-ban"></i> BLOCKED BY WELFARE</div>
                    <?php if (!empty($w['blocked_source'])): ?>
                      <div style="font-size: 11px; color: #991b1b; margin-top: 2px;">Source: <?= htmlspecialchars($w['blocked_source']) ?></div>
                    <?php endif; ?>
                  </div>
                <?php endif; ?>
                ...
                <div style="margin-bottom: 6px; font-size: 12px;">
                  ...
                  <?php if (!empty($w['executing_officer_code'])): ?>
                    <?php
                      ...
                      $eoBadge = $eoIsApproved ? 'text-success' : ($eoStatus === 'rejected' ? 'text-danger' : 'text-warning');
                      $eoLabel = $eoIsApproved ? 'Approved' : ($eoStatus === 'rejected' ? 'Rejected' : 'Pending');
                    ?>
                ...
                <div style="margin-bottom: 6px; font-size: 12px;">
                  ...
                  <strong class="booking-status <?= $bookingClass ?>" style="padding:0; background:none; font-size:12px; border:none; display:inline;"><?= htmlspecialchars($bookingLabel) ?></strong>
                  <div class="booking-meta" style="margin-top: 2px;">
                    <?= !empty($w['latest_training_batch']) ? 'Batch: ' . htmlspecialchars($w['latest_training_batch']) : 'Batch: Not booked' ?>
                    <?= !empty($w['latest_training_date']) ? ' | Date: ' . htmlspecialchars(date('d-m-Y', strtotime($w['latest_training_date']))) : '' ?>
                    | Attempts left: <?= (int)$attemptsLeft ?>
                  </div>
                  <?php if ($safetyEnrollmentStatus === 'approved' && !in_array($bookingClass, ['pass'], true)): ?>
                  ...
                  <?php elseif ($safetyEnrollmentStatus === 'rejected'): ?>
                    <div style="font-size:11px; color:#b91c1c; margin-top:2px;">Edit worker to resubmit.</div>
                  <?php endif; ?>
                </div>

                <!-- Safety Approval -->
                <div style="font-size: 12px;">
                  <span class="text-muted">Safety Approval:</span>
                  <?php
              ...
              <?php
                // Determine if this worker's form is locked (submitted, in approval pipeline)
                // Worker is editable ONLY when rejected OR never submitted (draft)
                $latestReqStatus  = strtolower(trim((string)($w['latest_training_request_status'] ?? '')));
                $eoStatus2        = strtolower(trim((string)($w['execution_training_status'] ?? '')));
                $safetyStatus2    = strtolower(trim((string)($w['safety_enrollment_status'] ?? '')));
                $hasTempId        = !empty($w['temp_id']);  // temp_id is only generated on Submit (not Save Draft)

                // Rejected by EO or Safety → editable (contractor can correct and resubmit)
                $isRejected = $eoStatus2 === 'rejected'
                           || $safetyStatus2 === 'rejected'
                           || $latestReqStatus === 'safety_rejected';

                // Worker is submitted if they have a temp_id OR a non-empty training request status
                // OR their execution_training_status is something other than empty/pending
                $isSubmitted = $hasTempId
                            || ($latestReqStatus !== '')
                            || !in_array($eoStatus2, ['', 'pending'], true);

                // Locked = submitted AND not rejected
                $workerIsLocked = $isSubmitted && !$isRejected;
              ?>
              <td>
      ...
      <div class="card">
        ...
        <script>
          document.addEventListener("DOMContentLoaded", function() {
            ...
            if (formSec && listSec && listSec.parentNode) {
        ...
        <form id="enrollForm" enctype="multipart/form-data">
          ...
          <div class="modal-tab-content" id="tab-basic">
            <div class="form-grid-3">
              ...
              <div class="form-group" id="expectedJoiningDateGroup">
                <label class="form-label required">Expected Join Date</label>
                <input type="date" class="form-control" name="expected_joining_date" id="expectedJoiningDateInput" required min="<?= date('Y-m-d') ?>">
              </div>
              <div class="form-group">
            ...
            <div class="form-grid-3">
              ...
              ?>
                ...
                <?php foreach ($dynamicDocs as $doc):
                  $reqAttr = $doc['is_mandatory'] ? 'required' : '';
                  $reqStar = $doc['is_mandatory'] ? '<span class="text-danger">*</span>' : '';
                ?>
                  <div class="doc-card">
                    <label class="form-label <?= $doc['is_mandatory'] ? 'required' : '' ?>"><?= htmlspecialchars($doc['document_type']) ?></label>
                    <?php if (!empty($doc['format_file_path'])): ?>
                      <div style="margin-bottom: 5px;">
                        <a href="<?= htmlspecialchars($doc['format_file_path']) ?>" target="_blank" class="text-primary" style="font-size: 12px; font-weight: 600;"><i class="fas fa-download"></i> Download Format</a>
                      </div>
                    <?php endif; ?>
                    <input type="file" class="form-control" name="dynamic_doc_<?= htmlspecialchars($doc['upload_key']) ?>" accept=".pdf,application/pdf" data-max-size="5242880" <?= $reqAttr ?>>
                    <small class="form-hint"><?= htmlspecialchars($doc['hint'] ?: 'PDF only, max 5 MB.') ?></small>
                  </div>
                <?php endforeach; ?>
              <?php endif; ?>
            </div>
    ...
    const listSection = document.getElementById('listSection');
        ...
        const dynamicDocsList = <?= json_encode($dynamicDocs ?: [], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?>;
        ...
        document.getElementById('btnOpenModal').onclick = () => {
          ...
          const expectedJoinInput = document.getElementById('expectedJoiningDateInput');
          if (expectedJoinInput) {
            expectedJoinInput.setAttribute('min', new Date().toISOString().slice(0, 10));
          }
        ...
        function activateTab(tabId) {
          ...
          if (tabId === 'training') {
            if (isPwoWorkOrder() && pwoPaymentCompleted && !hasShownPaymentSuccessPopup) {
              hasShownPaymentSuccessPopup = true;
              Swal.fire({
                icon: 'success',
                title: 'Payment Completed',
                text: 'Payment completed successfully. You may now proceed with the safety booking.',
        ...
        // Show lock message when form is non-editable
        function showLockedMsg(info) {
          const statusLabels = {
            'pending_eo'           : 'EO Approval Pending',
            'pending_safety'       : 'Safety Approval Pending',
            'welfare_pending'      : 'Safety Approval Pending',
          ...
          const label = statusLabels[info.status] || info.status.replace(/_/g,' ').toUpperCase();
          Swal.fire({
            icon : 'info',
            title: '\uD83D\uDD12 Enrollment Locked',
            html : `<div style="text-align:left;font-size:14px;line-height:1.8;">
                      <p>The enrollment for <strong>${info.name}</strong> has been <strong>submitted</strong> and is currently under review.</p>
                      <div style="background:#f1f5f9;border:1px solid #cbd5e1;border-radius:8px;padding:10px 14px;margin-top:8px;">
                        <span style="font-size:12px;color:#64748b;font-weight:700;">CURRENT STATUS</span><br>
                        <span style="font-size:15px;font-weight:800;color:#2563eb;">${label}</span>
                      </div>
                      <p style="margin-top:12px;font-size:13px;color:#64748b;">Editing is <strong>not allowed</strong> while the enrollment is under approval. The form will become editable again only if the enrollment is <strong>rejected</strong> by the Executing Officer or Safety Department.</p>
                    </div>`,
        ...
        function editWorker(worker) {
          ...
          pwoPaymentCompleted = !!(worker.is_safety_paid && (parseInt(worker.is_safety_paid) || worker.is_safety_paid === '1' || worker.is_safety_paid === true));
          hasShownPaymentSuccessPopup = false;
          ...
          const expectedJoinInput = document.getElementById('expectedJoiningDateInput');
          if (expectedJoinInput) expectedJoinInput.removeAttribute('min');
          ...
          if (requestedPassType === 'Workman' && typeof dynamicDocsList !== 'undefined') {
            dynamicDocsList.forEach(doc => {
              const name = 'dynamic_doc_' + doc.upload_key;
              const input = form.querySelector(`[name="${name}"]`);
              const fileName = (worker.documents && worker.documents[doc.document_type]) || '';
              if (input) input.dataset.existing = fileName || '';
            });
          }
        ...
        let hasShownPaymentSuccessPopup = false;
        ...
        function isPwoPayNow() {
          return isPwoWorkOrder() && selectedSafetyFeeOption() === 'pay_now';
        }
        ...
        function previewFiles() {
          ...
          if (requestedPassType === 'Workman' && typeof dynamicDocsList !== 'undefined') {
            dynamicDocsList.forEach(doc => {
              fileLabels.push(['dynamic_doc_' + doc.upload_key, doc.document_type]);
            });
          }
          const isEdit = Boolean(document.getElementById('workerEditId')?.value);
              ...
              return `
                <div class="preview-item">
                  ...
                  <div class="preview-doc-value">
                    ...
                    <button type="button" class="btn-view" onclick="viewPreviewDocument('${escapePreviewHtml(item.fileInputName || '')}', '${escapePreviewHtml(item.viewUrl || '')}')"><i class='fas fa-eye'></i> View</button>
                  </div>
                </div>
              `;
            }
        ...
        function showSubmitPreview() {
          ...
          const sections = [
            renderPreviewSection('Basic Info', [
              ...
              ['Expected Join Date', previewValue('expected_joining_date')],
          ...
          try {
            ...
            if (result.success) {
              ...
              if (result.payment && result.payment.payment_link) {
                if (selectedSafetyFeeOption() === 'pay_later') {
                  successMessage += '\nSafety Fee Payment: Pending';
                } else {
    ...
    function viewWorker(w) {
      ...
      const photoSrc = w.photo ? `../../uploads/workers/${w.photo}` : 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23cbd5e1"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
      ...
      let vaultHtml = '';
      vaultHtml += renderDocLink('Workman Photo', w.photo);
      vaultHtml += renderDocLink('Signature', w.signature);
      vaultHtml += renderDocLink('Aadhaar Card', w.aadhaar_doc);
      vaultHtml += renderDocLink('Training Approval', w.training_approval_doc);
      vaultHtml += renderDocLink('Education Cert', w.education_doc);
      vaultHtml += renderDocLink('Bank Passbook', w.bank_doc);
      vaultHtml += renderDocLink('Skill Certificate', w.skill_cert_doc);
      vaultHtml += renderDocLink('Police Verification', w.police_doc);
      vaultHtml += renderDocLink('Medical Report', w.medical_doc);
      vaultHtml += renderDocLink('Insurance Copy', w.insurance_doc);
        ...
        <div class="profile-container">
          ...
          <div class="profile-header-card">
            ...
            <div class="profile-header-info">
              ...
              <div class="profile-sub">${w.gender} | DOB: ${w.dob} | Reg Date: ${w.registration_date || 'N/A'}</div>
              <div class="badge-container">
                <span class="profile-badge badge-role"><i class="fas fa-user-shield"></i> ${w.pass_type}</span>
          ...
          <div class="profile-body-grid">
              ...
              <div class="profile-section-card">
                ...
                <div class="details-grid">
                  ...
                  <div class="detail-row">
                    ...
                    <span class="detail-val">${w.nature_of_work}</span>
                  </div>
                  <div class="detail-row">
                  ...
                  <div class="detail-row">
                    ...
                    <span class="detail-val">${w.esi_no || 'N/A'}</span>
                  </div>
                  <div class="detail-row">
                  ...
                  <div class="detail-row">
                    ...
                    <span class="detail-val" style="color:#10b981;">${w.expected_joining_date || 'N/A'}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="profile-section-card">
                ...
                <!-- Executing Officer Approval -->
                <div style="font-weight:700; font-size:11px; color:#64748b; margin-bottom:4px; text-transform:uppercase;">1. Executing Officer</div>
                <div class="approval-status-box ${eoStatusClass}">
                  <div style="display:flex; justify-content:space-between; font-weight:700; margin-bottom:4px;">
                    <span>STATUS:</span>
                    <span>${eoStatusLabel}</span>
                  </div>
                  <div><strong>Officer:</strong> ${(w.executing_officer_code || '-')} ${(w.executing_officer_name ? ' - ' + w.executing_officer_name : '')}</div>
                  <div style="margin-top:2px;"><strong>Remarks:</strong> ${w.execution_training_remarks || 'No remarks.'}</div>
                </div>

                <!-- Safety Department Document Verification -->
                <div style="font-weight:700; font-size:11px; color:#64748b; margin-bottom:4px; text-transform:uppercase; margin-top:12px;">2. Safety Enrollment</div>
                <div class="approval-status-box ${safetyStatusClass}">
                  <div style="display:flex; justify-content:space-between; font-weight:700; margin-bottom:4px;">
                    <span>STATUS:</span>
                    <span>${safetyStatusLabel}</span>
                  </div>
                  <div><strong>Remarks:</strong> ${w.safety_enrollment_remarks || 'No remarks.'}</div>
                </div>

                <!-- Safety Training Info -->
                <div style="font-weight:700; font-size:11px; color:#64748b; margin-bottom:4px; text-transform:uppercase; margin-top:12px;">3. Safety Training</div>
                <div style="padding:10px 12px; background:#f1f5f9; border-radius:12px; border:1px solid #cbd5e1; font-size:12px;">
                  ...
                  <div><strong>Date:</strong> ${w.latest_training_date ? w.latest_training_date : 'N/A'}</div>
    ...
    <!-- ===== RE-ENROLL MODAL ===== -->
    <div id="reEnrollModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.65); backdrop-filter:blur(4px); z-index:10999; align-items:center; justify-content:center; padding:20px;">
      <div style="background:#fff; border-radius:20px; width:min(100%,500px); box-shadow:0 25px 60px rgba(0,0,0,.3); overflow:hidden; animation:modalIn .25s ease;">
        <div style="background:linear-gradient(135deg,#f59e0b,#d97706); padding:20px 24px; display:flex; align-items:center; gap:14px;">
          ...
          <div>
            <div style="color:white;font-size:16px;font-weight:700;">Re-enroll for Training</div>
            <div style="color:rgba(255,255,255,.8);font-size:12px;" id="reEnrollSubtitle">Choose how to re-enroll this worker</div>
          </div>
          <button onclick="closeReEnrollModal()" style="margin-left:auto;background:rgba(255,255,255,.2);border:none;width:32px;height:32px;border-radius:8px;color:white;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;">&times;</button>
        </div>
        <div style="padding:24px;">
          <div id="reEnrollWorkerInfo" style="background:#fffbeb;border:1px solid #fcd34d;border-radius:10px;padding:14px;margin-bottom:20px;">
            <div style="font-weight:700;font-size:14px;" id="reEnrollWorkerName"></div>
            <div style="font-size:12px;color:#92400e;margin-top:4px;">Current PWO/PO: <strong id="reEnrollWorkOrder"></strong></div>
            <div style="font-size:11px;color:#78716c;margin-top:2px;" id="reEnrollAttemptsInfo"></div>
          </div>
          <p style="font-size:13px;color:#374151;margin-bottom:18px;">This worker failed their last training. How would you like to re-enroll?</p>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
            <button id="btnSamePO" onclick="doReEnroll('same_po')" style="padding:16px 12px;border:2px solid #6366f1;border-radius:12px;background:#f0f0ff;color:#4338ca;font-weight:700;font-size:13px;cursor:pointer;text-align:center;transition:all .2s;">
              <i class="fas fa-link" style="display:block;font-size:22px;margin-bottom:8px;color:#6366f1;"></i>
              Same PWO/PO<br>
              <small style="font-weight:400;color:#6366f1;font-size:11px;">Re-queue directly into training</small>
            </button>
            <button id="btnFreshPO" onclick="doReEnroll('fresh_po')" style="padding:16px 12px;border:2px solid #10b981;border-radius:12px;background:#f0fdf4;color:#065f46;font-weight:700;font-size:13px;cursor:pointer;text-align:center;transition:all .2s;">
              <i class="fas fa-plus-circle" style="display:block;font-size:22px;margin-bottom:8px;color:#10b981;"></i>
              Fresh PWO/PO<br>
              <small style="font-weight:400;color:#10b981;font-size:11px;">Fill new work order details</small>
            </button>
          </div>
        </div>
      </div>
    ...
    <script>
    let _reEnrollWorker = null;

    function openReEnrollModal(worker) {
        _reEnrollWorker = worker;
        document.getElementById('reEnrollWorkerName').textContent = worker.name;
        const wo = worker.work_order_no || '—';
        const src = worker.work_order_source ? ' [' + worker.work_order_source + ']' : '';
        document.getElementById('reEnrollWorkOrder').textContent = src + wo;
        document.getElementById('reEnrollAttemptsInfo').textContent = 'Attempts remaining: ' + worker.attempts_left + ' of 3';
        document.getElementById('reEnrollSubtitle').textContent = 'Choose how to re-enroll ' + worker.name;
    ...
    function closeReEnrollModal() {
        ...
        _reEnrollWorker = null;
    }

    async function doReEnroll(mode) {
        if (!_reEnrollWorker) return;
        const worker = _reEnrollWorker;

        if (mode === 'fresh_po') {
            // Close modal and trigger the edit form for re-enrolment with fresh PO
            closeReEnrollModal();
            if (typeof editWorker === 'function') {
                // Scroll to form and open edit with this worker
                editWorker(worker);
                // Show hint after short delay
                setTimeout(() => {
                    const woSelect = document.getElementById('workOrderSelect');
                    if (woSelect) {
                        woSelect.style.border = '2px solid #10b981';
                        woSelect.style.boxShadow = '0 0 0 3px rgba(16,185,129,.2)';
                        woSelect.title = 'Please select new PWO/PO for re-enrolment';
                    }
                }, 500);
            } else {
                window.location.href = 'enrolment-4a.php?type=workmen#formSection';
            }
            return;
        }

        // Same PO → API call
        const btn = document.getElementById('btnSamePO');
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Processing…';

        try {
            const fd = new FormData();
            fd.append('workman_id', worker.id);
            fd.append('mode', 'same_po');
            const res = await fetch('../../api/contractor/re_enroll_training.php', { method: 'POST', body: fd });
            const data = await res.json();
            ...
            if (data.success) {
                ...
                if (typeof Swal !== 'undefined') {
                    Swal.fire({ icon: 'error', title: 'Error', text: data.error || 'Re-enrolment failed.', confirmButtonColor: '#ef4444' });
                } else {
                    alert('Error: ' + (data.error || 'Re-enrolment failed.'));
                }
            }
        } catch (e) {
            closeReEnrollModal();
            if (typeof Swal !== 'undefined') {
                Swal.fire({ icon: 'error', title: 'Network Error', text: 'Could not connect. Please try again.', confirmButtonColor: '#ef4444' });
            } else {
                alert('Network error. Please try again.');
            }
        } finally {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-link" style="display:block;font-size:22px;margin-bottom:8px;color:#6366f1;"></i>Same PWO/PO<br><small style="font-weight:400;color:#6366f1;font-size:11px;">Re-queue directly into training</small>';
        }
    }
