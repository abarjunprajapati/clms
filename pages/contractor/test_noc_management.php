...
function renderContent() {
    ...
    echo "<h1>CONTENT_START</h1>";
    try {
        // Fetch incoming NOC requests
        $in_req = $conn->prepare("
    ...
    ");
    $in_req->bind_param("i", $contractor_id);
    $in_req->execute();
    $res = $in_req->get_result();
    $incoming_requests = $res ? $res->fetch_all(MYSQLI_ASSOC) : [];
    ...
    $outgoing_requests = $res2 ? $res2->fetch_all(MYSQLI_ASSOC) : [];

    ?>
    <style>
        .page-content { padding: 24px; max-width: 1200px; margin: 0 auto; }
        .req-card { background: white; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; margin-bottom: 24px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); }
        .req-card h3 { margin-top: 0; color: #1e293b; font-size: 18px; margin-bottom: 16px; border-bottom: 1px solid #f1f5f9; padding-bottom: 12px; }
    ...
    <div class="page-content">
        <div style="display:flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
            <div>
                <h1 style="font-size: 24px; color: #0f172a; margin: 0 0 8px 0;">NOC Management</h1>
        ...
        <div class="req-card">
            <h3><i class="fas fa-inbox text-blue-600"></i> Incoming Requests (Other contractors want to hire your workmen)</h3>
            <div class="tbl-responsive">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr><td colspan="6" style="text-align: center; color: #94a3b8; padding: 32px;">No incoming requests found.</td></tr>
                        <?php else: ?>
        ...
        <div class="req-card" style="margin-top: 30px;">
            <h3><i class="fas fa-paper-plane text-indigo-600"></i> Outgoing Requests (You requested these workmen)</h3>
            <div class="tbl-responsive">
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr><td colspan="4" style="text-align: center; color: #94a3b8; padding: 32px;">No outgoing requests found.</td></tr>
                        <?php else: ?>
    ...
    <script>
        ...
        async function submitNocRequest() {
            ...
            try {
                const res = await fetch('../../api/contractor/noc_request.php', {
                    method: 'POST',
        ...
        async function respondNoc(id, action) {
            ...
            if(result.isConfirmed) {
                try {
                    const res = await fetch('../../api/contractor/noc_respond.php', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
