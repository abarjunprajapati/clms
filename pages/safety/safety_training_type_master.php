...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (($_POST['action'] ?? '') === 'set_status') {
            ...
            clms_set_training_type_status($conn, (int)($_POST['id'] ?? 0), $status);
            $_SESSION['training_type_message'] = ['type' => 'success', 'text' => 'Training type set ' . $status . '.'];
        } else {
            clms_add_training_type($conn, $_POST['type_name'] ?? '', $_POST['duration_hours'] ?? 8, $_POST['pass_mark'] ?? 60, $_POST['from_date'] ?? date('Y-m-d'), $_POST['to_date'] ?? '9999-12-31', (int)($_POST['id'] ?? 0), $_POST['status'] ?? 'active');
