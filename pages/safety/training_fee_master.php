...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        db_execute($conn, "DELETE FROM training_fee_masters WHERE id = ?", 'i', array((int)($_POST['id'] ?? 0)));
    } else {
        $id = (int)($_POST['id'] ?? 0);
        ...
        if (in_array($source, array('PWO', 'PO', 'SO'), true)) {
            if ($id > 0) {
                db_execute($conn, "UPDATE training_fee_masters SET fee_source = ?, amount = ?, from_date = ?, to_date = ?, status = ?, updated_at = NOW() WHERE id = ?", 'sdsssi', array($source, max(0, (float)($_POST['amount'] ?? 0)), $fromDate, $toDate, $_POST['status'] ?? 'active', $id));
            } else {
                db_execute($conn, "INSERT INTO training_fee_masters (fee_source, amount, from_date, to_date, status, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW()) ON DUPLICATE KEY UPDATE amount = VALUES(amount), from_date = VALUES(from_date), to_date = VALUES(to_date), status = VALUES(status), updated_at = NOW()", 'sdsssi', array($source, max(0, (float)($_POST['amount'] ?? 0)), $fromDate, $toDate, $_POST['status'] ?? 'active', (int)($_SESSION['user_id'] ?? 0)));
            }
        }
    }
