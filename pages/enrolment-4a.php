...
function enrolment_worker_type_condition($conn, $table, $type) {
    ...
    $map = [
        ...
        'workmen' => ['workman', 'Workman', 'workmen', 'Workmen', 'Workman Pass', 'Workmen Pass'],
        'workman' => ['workman', 'Workman', 'workmen', 'Workmen', 'Workman Pass', 'Workmen Pass']
    ];
    $values = $map[$type] ?? [];
    if (!$values) {
        return '1=1';
...
function renderContent() {
    ...
    if (!empty($workerIds)) {
        ...
        while ($docRow = mysqli_fetch_assoc($docRes)) {
            $workerDocs[$docRow['workman_id']][] = [
                'type' => $docRow['document_type'],
                'file_path' => basename($docRow['file_path'])
            ];
        }
    }
      ...
      <div class="card-body p-3">
        <style>
          .dashboard-metrics-container {
              ...
              gap: 8px;
              width: 100%;
              overflow-x: hidden; /* disable scrollbar as requested */
          }
          .stats-grid {
          ...
          #passLimitsWidget > div {
              display: flex !important;
              flex-wrap: nowrap !important;
              gap: 8px !important;
          }
          #passLimitsWidget > div > div {
              flex: 1 1 0;
              min-width: 0;
          }
          /* Hide the text when loading so it doesn't break flex layout badly */
          #passLimitsWidget p {
              margin: 0;
              padding: 6px;
          }
        ...
        <div class="dashboard-metrics-container">
          ...
          <!-- Pass Limits -->
          <div id="passLimitsWidget" style="font-size:12px;">
        ...
        <form id="enrollForm" enctype="multipart/form-data">
          ...
          <div class="modal-tab-content hidden" id="tab-payment">
            <div class="training-booking-box" id="pwoPaymentOptionBox" style="margin-bottom:12px;display:none;">
              ...
              <label class="choice-row">
                <input type="radio" name="safety_fee_payment_option" value="pay_later" checked>
    ...
    const listSection = document.getElementById('listSection');
        ...
        document.getElementById('btnOpenModal').onclick = () => {
          ...
          // Collapse sidebar for full focus
        ...
        document.getElementById('btnNextTab').onclick = () => {
          ...
          const currentSection = document.getElementById(`tab-${current}`);
          if (currentSection) {
            ...
            for (let i = 0; i < requiredFields.length; i++) {
              ...
              if (!field.value || field.value.trim() === '') {
                let labelName = field.name || 'This field';
                const labelElement = field.closest('.form-group')?.querySelector('label') || currentSection.querySelector(`label[for="${field.id}"]`);
                if (labelElement) {
                  labelName = labelElement.textContent.replace(/\*/g, '').trim();
                } else if (field.placeholder) {
                  labelName = field.placeholder;
                }
                ...
                if (typeof Swal !== 'undefined' && Swal.fire) {
                  Swal.fire({
                    ...
                    text: `Please fill out: ${labelName}`,
                    confirmButtonColor: '#1e293b'
                  }).then(() => {
                    field.focus();
                  });
                } else {
                  alert(`Please fill out: ${labelName}`);
                  field.focus();
                }
                return;
              }
        ...
        async function submitEnrollment(action = 'submit', confirmedPreview = false) {
          ...
          try {
            ...
            if (result.success) {
              ...
              let tBooking = 'Not booked.';
              if (bookingChoice === 'book_now') {
                const trainingDateInput = document.getElementById('safetyTrainingDate');
                let tDate = '';
                if (trainingDateInput && trainingDateInput.value) {
                  const d = new Date(trainingDateInput.value);
                  if (!isNaN(d)) {
                    tDate = d.toLocaleDateString('en-GB', {day: '2-digit', month: 'short', year: 'numeric'});
                  }
                }
                tBooking = tDate ? `Booking done for ${tDate}.` : 'Booking done.';
              }
              ...
              if (result.payment && result.payment.payment_link) {
                if (selectedSafetyFeeOption() === 'pay_later') {
                  ...
                  tFee = `Paid (Ref: ${result.payment.payment_ref}).`;
                }
              }

              let htmlMessage = `
                <div style="font-size: 15px; text-align: left; background: #eff6ff; color: #1e3a8a; padding: 15px; border-radius: 8px; border: 1px solid #bfdbfe; margin-top: 10px; font-family: sans-serif;">
                  <table style="width: 100%; border-collapse: collapse;">
                    <tr>
                      <td style="padding: 4px 10px 4px 0; font-weight: 700; width: 45%; color: #1e40af; vertical-align: top;">Temp ID:</td>
                      <td style="padding: 4px 0; color: #1e3a8a; vertical-align: top;">${result.temp_id || 'N/A'}.</td>
                    </tr>
                    <tr>
                      <td style="padding: 4px 10px 4px 0; font-weight: 700; color: #1e40af; vertical-align: top;">Safety Training:</td>
                      <td style="padding: 4px 0; color: #1e3a8a; vertical-align: top;">${tBooking}</td>
                    </tr>
                    <tr>
                      <td style="padding: 4px 10px 4px 0; font-weight: 700; color: #1e40af; vertical-align: top;">Safety Training Fee:</td>
                      <td style="padding: 4px 0; color: #1e3a8a; vertical-align: top;">${tFee}</td>
                    </tr>
                  </table>
                </div>
              `;
              ...
              if (typeof Swal !== 'undefined' && Swal.fire) {
                ...
                alert("Enrollment Submitted! Temp ID: " + result.temp_id);
                afterSuccess();
              }
            } else {
