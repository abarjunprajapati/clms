...
$sql = "INSERT INTO execution_observations (execution_officer_id, contractor_id, workman_id, work_order_id, observation_type, remarks, severity, action_required) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
if ($success) {
    ...
    // Audit Log
    db_execute($conn, "INSERT INTO execution_audit_logs (execution_officer_id, action, entity_type, entity_id, new_value) VALUES (?, ?, ?, ?, ?)", [
    ...
    if ($severity === 'high' || $action_required) {
        db_execute($conn, "INSERT INTO execution_notifications (execution_officer_id, recipient_role, title, message) VALUES (?, ?, ?, ?)", [
