...
$res = mysqli_query($conn, "SELECT id, contractor_name, labour_validity, contractor_id FROM annexure2a WHERE labour_validity <= '$alertDate'");
...
$res = mysqli_query($conn, "SELECT id, insurance_validity_to, contractor_id FROM annexure3a WHERE insurance_validity_to <= '$alertDate'");
while ($c = mysqli_fetch_assoc($res)) {
