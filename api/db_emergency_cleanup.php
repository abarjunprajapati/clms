...
foreach ($profiles as $profile) {
    ...
    $lastError = $conn ? mysqli_connect_error() : 'mysqli_init failed';
...
$user = mysqli_real_escape_string($conn, $used['user']);
$db = mysqli_real_escape_string($conn, $used['db']);
...
if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if (@mysqli_query($conn, "KILL {$id}")) {
            $killed[] = $id;
        } else {
            $errors[] = "KILL {$id}: " . mysqli_error($conn);
        }
    }
} else {
...
$status = @mysqli_query($conn, "SHOW STATUS WHERE Variable_name IN ('Threads_connected','Threads_running','Max_used_connections')");
$statusRows = [];
if ($status) {
...
$variables = @mysqli_query($conn, "SHOW VARIABLES WHERE Variable_name IN ('max_connections','wait_timeout','interactive_timeout')");
$variableRows = [];
if ($variables) {
