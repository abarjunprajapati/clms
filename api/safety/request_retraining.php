...
if (db_execute($conn, $sql, 'i', [$workman_id])) {
    ...
    if (clms_db_query($conn, "SHOW TABLES LIKE 'training_requests'") && requestRetrainingColumnExists($conn, 'training_requests', 'status')) {
    ...
    header("Location: ../../pages/safety/retraining.php?error=" . clms_db_error($conn));
}
