...
try {
    ...
    if ($action === 'generate') {
        ...
        $safetyResult = clms_db_query($conn, $safetyCheck);
        $safetyData = clms_db_fetch_assoc($safetyResult);
        ...
        if (!clms_db_query($conn, $insertQuery)) {
            throw new Exception("Failed to generate pass: " . clms_db_error($conn));
        }
    ...
    elseif ($action === 'print') {
        ...
        $printer = isset($_POST['printer']) ? clms_db_real_escape_string($conn, $_POST['printer']) : 'Default Printer';
