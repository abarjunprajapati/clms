...
try {
    ...
    if ($action === 'assign_batch') {
        ...
        // Verify batch exists
        $batchCheck = mysqli_query($conn, "SELECT * FROM safety_batches WHERE batch_id = $batch_id FOR UPDATE");
        if (mysqli_num_rows($batchCheck) === 0) throw new Exception("Batch not found.");
        $batch = mysqli_fetch_assoc($batchCheck);
        ...
        if (!mysqli_query($conn, $insertQuery)) {
            throw new Exception("Failed to assign safety batch: " . mysqli_error($conn));
        }
    ...
    elseif ($action === 'update_result') {
        ...
        $result_status = isset($_POST['result_status']) ? mysqli_real_escape_string($conn, $_POST['result_status']) : ''; // Passed/Failed
