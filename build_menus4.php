...
foreach($lines as $line) {
    // STOP parsing once we hit the Granted Access renderer block
    if (strpos($line, '$specialLinks') !== false || strpos($line, '$dynamicModules[$mod]') !== false) {
        break;
    }
    ...
    if (strpos($line, '<a href=') !== false && strpos($line, 'class="sidebar-item"') !== false) {
        ...
        if (strpos($line, '$m[\'link\']') !== false || strpos($line, '$m[link]') !== false) continue;
